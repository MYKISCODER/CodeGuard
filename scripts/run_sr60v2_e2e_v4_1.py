"""
CodeGuard — SemiReal-60 v2 E2E Runner with Layer2 v4
...
def run():
    ...
    print(f"Using Layer2 v4.1 prompt: CI Install Dependency Mapping Fix\n")
    ...
    # Save raw
    raw_path = OUTPUT_DIR / "sr60v2_layer2_v4_1_raw.json"
    ...
    for mode in MODES:
        csv_path = OUTPUT_DIR / f"sr60v2_e2e_v4_1_{mode.lower().replace('-','_')}.csv"
    ...
    # Summary
    print("\n" + "=" * 80)
    print("SEMIREAL-60 v2 E2E V4 SUMMARY (Conservative)")
    print("=" * 80)
    for mode in MODES:
        csv_path = OUTPUT_DIR / f"sr60v2_e2e_v4_{mode.lower().replace('-','_')}.csv"
