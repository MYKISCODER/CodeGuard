"""
CodeGuard — Prompt-only Layer2 Baseline Runner

Reads mini_e2e.yaml, calls OpenAI-compatible API with the Layer2 prompt,
feeds parsed behaviors into policy_engine, outputs CSV results.
"""
...
# Allow importing policy_engine from ../policy/
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "policy"))
from policy_engine import arbitrate_case

# ── Config ────────────────────────────────────────────────────────
BENCHMARK_PATH = os.path.join(os.path.dirname(__file__), "..", "benchmark", "mini_e2e.yaml")
...
def _extract_files_section(repo_snapshot):
    """Extract only the 'files:' section from repo_snapshot, excluding README and notes."""
...
def call_layer2(client, model, case):
    """Call the LLM with the Layer2 prompt and return parsed behaviors list."""
...
def run():
    ...
    # Collect Layer2 outputs first
    layer2_results = []
    for case in cases:
        cid = case["case_id"]
        print(f"  Calling Layer2 for {cid} ...")
    ...
    # Save raw Layer2 outputs
    raw_path = os.path.join(OUTPUT_DIR, "layer2_raw_v2.json")
    ...
    # Run Layer3 policy for each mode and write CSV
    for mode in MODES:
        csv_path = os.path.join(OUTPUT_DIR, f"e2e_v2_{mode.lower()}.csv")
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            ...
            for r in layer2_results:
                ...
                gold_obfusc = case["gold_expected_behaviors"][0].get("obfuscation_scope", "?")
                writer.writerow([
                    case["case_id"],
                    case["is_trap"],
