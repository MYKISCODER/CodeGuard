"""
CodeGuard — Full 50-case E2E Runner

Uses full_e2e.yaml, runs Layer2 v2 + policy for all 4 modes (STRICT, STRICT-EXEMPT, MODERATE, PERMISSIVE).
"""
...
BENCHMARK_PATH = os.path.join(os.path.dirname(__file__), "..", "benchmark", "full_e2e.yaml")
...
def run():
    ...
    for i, case in enumerate(cases):
        cid = case["case_id"]
        print(f"  [{i+1}/{len(cases)}] Calling Layer2 for {cid} ...")
    ...
    # Save raw
    raw_path = os.path.join(OUTPUT_DIR, "full_layer2_raw.json")
    ...
    for mode in MODES:
        csv_path = os.path.join(OUTPUT_DIR, f"full_e2e_{mode.lower().replace('-','_')}.csv")
    ...
    # Summary
    print("\n" + "=" * 80)
    print("FULL 50-CASE E2E SUMMARY")
    print("=" * 80)
    for mode in MODES:
        csv_path = os.path.join(OUTPUT_DIR, f"full_e2e_{mode.lower().replace('-','_')}.csv")
