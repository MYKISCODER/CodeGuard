"""
Cross-Model Validation v3 — Additional models

Supplements the existing cross-model results with newer/diverse models.
Uses the exact v4.1 prompt from the original evaluation.
"""
...
MODELS = [
    {"name": "gpt-4.1-mini", "model_id": "gpt-4.1-mini"},
    {"name": "deepseek-v3", "model_id": "deepseek-v3-250324"},
    {"name": "gemini-2.0-flash", "model_id": "gemini-2.0-flash"},
]

# Import the exact v4.1 prompt
...
def run_model(client, model_info, cases):
    ...
    for i, case in enumerate(cases):
        ...
        if "API_ERROR" in str(raw):
            errors += 1
            if errors > 10:
                print(f"  Too many errors ({errors}), skipping model")
                return None
...
def main():
    ...
    prev_path = OUTPUT_DIR / "cross_model_v2.csv"
    prev_results = []
    if prev_path.exists():
    ...
    # Save combined CSV
    csv_path = OUTPUT_DIR / "cross_model_all.csv"
    ...
    print(f"\n{'='*75}")
    print("CROSS-MODEL VALIDATION — ALL MODELS (v4.1 prompt, SR60v2, STRICT-EXEMPT)")
    print(f"{'='*75}")
