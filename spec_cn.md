...
Frozen Schema 中的每条 behavior 代表恰好一个**原子副作用事实**。然而，Layer 2 提取器（无论是 LLM 还是规则）可能偶尔产生非原子行为——例如，单条 behavior 同时包含 `action=EXEC_CMD` 和 `data_flow=UPLOAD_EXFIL`，将命令执行与数据外泄混为一体。

**规范化规则：** 在 Layer 3 仲裁之前，规范化步骤检查每条 behavior。如果一条 behavior 同时携带 `action=EXEC_CMD` 和 `data_flow=UPLOAD_EXFIL`，则拆分为两条原子 behavior：

1. **命令执行事实：** 原始 behavior，`data_flow` 覆写为 `NONE`
2. **外泄事实：** 原始 behavior，`action` 覆写为 `NETWORK_CONNECT`

这确保 Layer 3 规则（R1 处理外泄、R3 处理命令执行）各自作用于正确的原子事实，case 级别的 `max()` 聚合产生正确的推导权限（如 `max(L4, L3) = L4`）。

**设计理由：**
- **可审计性：** 两条原子 behavior 比一条混合 behavior 产生更清晰的审计追踪——每条规则触发于独立的事实。
- **规则可组合性：** Layer 3 的机制（白名单、混淆惩罚、不可解析目标升级）设计用于单维度行为。混合行为可能导致意外交互。
- **一致性：** case 级别的 OR/max 聚合天然处理多行为场景，规范化是无损变换。

**范围：** 此规范化不修改 Frozen Schema 的字段定义或枚举值。它是一个**契约强制执行**步骤，确保 Layer 2 输出在策略评估前符合原子行为不变量。

**实证验证：** 在 SemiReal-120 v1 上，Layer 2 v4.1 提取器产生零条混合行为（0/86 条 behavior 触发规范化）。规范化前后所有 120 个 case 的决策完全一致，确认对现有结果零影响。
...
1. **C-L-P 三维分类学：**
我们提出了首个面向代码智能体仓库级投毒攻击的生命周期感知与权限感知威胁模型，形式化为三维坐标系（载体 × 生命周期 × 权限）。
